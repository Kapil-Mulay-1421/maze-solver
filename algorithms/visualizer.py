...
def visualize_maze_gui(self, maze, positions, known_walls_at_each_step, save_dir="maze_runs", fps=10):
    walls = get_walls()
    wall_set = set(walls)
    ...
    run_num = 1
    while os.path.exists(os.path.join(save_dir, f"maze_run_{run_num}.gif")):
        run_num += 1
    save_path = os.path.join(save_dir, f"maze_run_{run_num}.gif")
    ...
    print(f"Run {run_num} finished. Saving run {run_num} to /maze_runs directory...")
    anim = animation.FuncAnimation(fig, draw_frame, frames=len(positions), interval=1000 / fps)
